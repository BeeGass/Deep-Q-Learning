...
class Agent():
  ...
  def SGD(self):
#self.agent.addToReplay((state, input_action, reward, grey_scaled_next_state, done)) e 17 at 9:52
    ...
    #for batch in mini_batch:
        ...
        #print("the reward: ", reward)
                ...
                #print("this is yj", yj)
            #loss = self.lossFn(predicted_reward.unsqueeze(0), yj.detach())
        #else:
            #target_q_val = self.model(next_state)
            #print("target_q_val: ", target_q_val)
            #yj += self.gamma * target_q_val[0][torch.argmax(target_q_val)]
            #loss = self.lossFn(predicted_reward.unsqueeze(0), yj.detach())
        #loss.backward()
        #self.optimizer.step()
...
class DQN():
    def __init__(self):
        ...
        self.transform = T.Compose([T.Grayscale(), T.Resize((64,64))])
    ...
    def execute_action(self, input_action, state):
        ...
        transition = (state, input_action, reward, grey_scaled_next_state, done)
        self.agent.addToReplay((state, input_action, reward, grey_scaled_next_state, done))
...
#             loss = -100000000000
            ...
            if (e + 1) % 10 == 0:
                ...
                self.save_weights(self.agent.replay_buffer_list, self.epsilon, self.agent.model.state_dict(), loss, scoreEachEp)
